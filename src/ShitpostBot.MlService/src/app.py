...
def _extract_ocr_text_tesseract(pil_img: Image.Image) -> Tuple[str, float]:
    ...
    data = pytesseract.image_to_data(
        pil_img, lang="eng+ces+slk", config=config, output_type=pytesseract.Output.DICT
    )
...
def _extract_ocr_texts_batch(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(_extract_ocr_text_tesseract, pil_img)
            for pil_img in pil_images
        ]
        results = []
...
def _generate_captions_batch(pil_images: List[Image.Image]) -> List[str]:
    ...
    captions = [
        blip_processor.decode(output, skip_special_tokens=True) for output in outputs
    ]
    return captions
...
async def process_image_batch(request: ProcessImageBatchRequest):
    ...
    ocr_results = None
    if request.ocr:
        ocr_results = _extract_ocr_texts_batch(
            cv_images, pil_images, request.use_tesseract
        )
