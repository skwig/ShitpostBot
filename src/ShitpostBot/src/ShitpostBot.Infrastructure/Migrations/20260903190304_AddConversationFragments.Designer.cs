// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using Pgvector;
using ShitpostBot.Infrastructure;

#nullable disable

namespace ShitpostBot.Infrastructure.Migrations
{
    [DbContext(typeof(ShitpostBotDbContext))]
    [Migration("20260903190304_AddConversationFragments")]
    partial class AddConversationFragments
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.HasPostgresExtension(modelBuilder, "vector");
            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("ShitpostBot.Domain.ConversationFragment", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<decimal>("ChannelId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<Vector>("Embedding")
                        .IsRequired()
                        .HasColumnType("vector(384)");

                    b.Property<DateTimeOffset>("EndedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("FirstMessageId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<decimal>("GuildId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<decimal>("LastMessageId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<int>("MessageCount")
                        .HasColumnType("integer");

                    b.Property<DateTimeOffset>("StartedAt")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id");

                    b.HasIndex("GuildId", "ChannelId", "FirstMessageId").IsUnique();

                    b.ToTable("ConversationFragment");
                });

            modelBuilder.Entity("ShitpostBot.Domain.DailySlopEntry", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<decimal>("ChatChannelId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<decimal>("ChatGuildId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<decimal>("ChatMessageId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<string>("GameId")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTimeOffset>("PostedOn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("PosterId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<DateTimeOffset>("TrackedOn")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id");

                    b.HasIndex("PostedOn");

                    b.HasIndex("PosterId");

                    b.HasIndex("ChatGuildId", "ChatChannelId", "ChatMessageId")
                        .IsUnique();

                    b.ToTable("DailySlopEntry", (string)null);
                });

            modelBuilder.Entity("ShitpostBot.Domain.Post", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<decimal>("ChatChannelId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<decimal>("ChatGuildId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<decimal>("ChatMessageId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<DateTimeOffset?>("EvaluatedOn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTimeOffset>("PostedOn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<decimal>("PosterId")
                        .HasColumnType("numeric(20,0)");

                    b.Property<DateTimeOffset>("TrackedOn")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("Type")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("ChatMessageId");

                    b.HasIndex("PostedOn");

                    b.HasIndex("PosterId");

                    b.ToTable("Post");

                    b.HasDiscriminator<int>("Type");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("ShitpostBot.Domain.WhitelistedPost", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("PostId")
                        .HasColumnType("bigint");

                    b.Property<decimal>("WhitelistedById")
                        .HasColumnType("numeric(20,0)");

                    b.Property<DateTimeOffset>("WhitelistedOn")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id");

                    b.HasIndex("PostId")
                        .IsUnique();

                    b.ToTable("WhitelistedPost");
                });

            modelBuilder.Entity("ShitpostBot.Domain.ImagePost", b =>
                {
                    b.HasBaseType("ShitpostBot.Domain.Post");

                    b.Property<bool>("IsPostAvailable")
                        .HasColumnType("boolean");

                    b.HasDiscriminator().HasValue(0);
                });

            modelBuilder.Entity("ShitpostBot.Domain.LinkPost", b =>
                {
                    b.HasBaseType("ShitpostBot.Domain.Post");

                    b.HasDiscriminator().HasValue(1);
                });

            modelBuilder.Entity("ShitpostBot.Domain.WhitelistedPost", b =>
                {
                    b.HasOne("ShitpostBot.Domain.ImagePost", "Post")
                        .WithOne()
                        .HasForeignKey("ShitpostBot.Domain.WhitelistedPost", "PostId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Post");
                });

            modelBuilder.Entity("ShitpostBot.Domain.ImagePost", b =>
                {
                    b.OwnsOne("ShitpostBot.Domain.Image", "Image", b1 =>
                        {
                            b1.Property<long>("ImagePostId")
                                .HasColumnType("bigint");

                            b1.Property<decimal>("ImageId")
                                .HasColumnType("numeric(20,0)");

                            b1.Property<string>("ImageUri")
                                .IsRequired()
                                .HasColumnType("text");

                            b1.Property<DateTimeOffset?>("ImageUriFetchedAt")
                                .HasColumnType("timestamp with time zone");

                            b1.Property<string>("MediaType")
                                .HasColumnType("text");

                            b1.HasKey("ImagePostId");

                            b1.HasIndex("ImageId")
                                .IsUnique();

                            b1.ToTable("Post");

                            b1.WithOwner()
                                .HasForeignKey("ImagePostId");

                            b1.OwnsOne("ShitpostBot.Domain.ImageFeatures", "ImageFeatures", b2 =>
                                {
                                    b2.Property<long>("ImagePostId")
                                        .HasColumnType("bigint");

                                    b2.Property<Vector>("FeatureVector")
                                        .IsRequired()
                                        .HasColumnType("vector");

                                    b2.Property<string>("ModelName")
                                        .IsRequired()
                                        .HasColumnType("text");

                                    b2.HasKey("ImagePostId");

                                    b2.ToTable("Post");

                                    b2.WithOwner()
                                        .HasForeignKey("ImagePostId");
                                });

                            b1.Navigation("ImageFeatures");
                        });

                    b.Navigation("Image")
                        .IsRequired();
                });

            modelBuilder.Entity("ShitpostBot.Domain.LinkPost", b =>
                {
                    b.OwnsOne("ShitpostBot.Domain.Link", "Link", b1 =>
                        {
                            b1.Property<long>("LinkPostId")
                                .HasColumnType("bigint");

                            b1.Property<string>("LinkId")
                                .HasColumnType("text");

                            b1.Property<int>("LinkProvider")
                                .HasColumnType("integer");

                            b1.Property<string>("LinkUri")
                                .IsRequired()
                                .HasColumnType("text");

                            b1.HasKey("LinkPostId");

                            b1.ToTable("Post");

                            b1.WithOwner()
                                .HasForeignKey("LinkPostId");
                        });

                    b.Navigation("Link")
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
