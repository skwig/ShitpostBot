...
) : BotCommandFeature(chatClient)
{
    public override string? HelpMessage =>
        "`repost unwhitelist` - removes a post from the whitelist";
    ...
    )
    {
        ...
        {
            await chatClient.SendMessage(destination, "This post is not whitelisted");
        ...
        await chatClient.SendMessage(destination, "Unwhitelisted");
